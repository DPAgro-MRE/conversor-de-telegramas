...
def gerarExcel(dados):
    try:
        ...
        for row in planilha.iter_rows(min_row=2, max_row=planilha.max_row, max_col=planilha.max_column):
            ...
            if any(str(cell.value).lower() == "reservado" for cell in row):
                # Aplica a fonte vermelha para todas as células da linha
                for cell in row:
                    if str(cell.value).lower() == "reservado":
...
def gerarTxt(textoPdf, NumTel, Posto):
    #Função de debug, usada principalmente para os Reservados
    try:
        pasta_downloads = os.path.join(os.path.expanduser("~"), "Downloads")
        nome_arquivo_txt = f"TEL{NumTel}-{Posto}.txt"
        caminho_txt = os.path.join(pasta_downloads, nome_arquivo_txt) 
        with open(caminho_txt, 'w', encoding='utf-8-sig') as arquivo_txt:
...
def Extracao(filepath, geraCsv, geraExcel, DPAgro, Reservado):
    ...
    # Lista com os padrões de texto para minutas de telegramas reservados
    ...
    for pagina in TELsValidos:
        ...
        if int(numero_pagina[0])//int(numero_pagina[1]) != 1:
            ...
            if int(numero_pagina[0]) == 1 and int(numero_pagina[1]) == 1 and len(TEL[0].split("\n")) <= 7: #Se o número da página for 1/1 e a quantidade de linhas for menor ou igual a 7, a página veio com defeito e será ignorada.
                ...
                Carater = match_carater.group(1).title()
                Prioridade = match_prioridade.group(1).title()
                ...
                if DPAgro == 1 and (match_distribuicao.group(1).lower().startswith("dpagro") or Redistribuicao.lower().startswith("dpagro")):
                    if Carater.lower() == 'reservado' and Reservado == 1:
