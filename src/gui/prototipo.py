    ...
    for pagina in TELsValidos:
        ...
        if int(numero_pagina[0])//int(numero_pagina[1]) != 1:
            ...
            if match_distribuicao.group(1).lower().startswith("dpagro") or Redistribuicao.lower().startswith("dpagro"):
                ...
                data_e_hora = f"{data_recebimento[6:] + '-' + data_recebimento[3:5] + '-' + data_recebimento[:2]}T{hora_entrada}Z" #CHECK
                ...
                if Carater == 'Reservado':
                    ...
                    Teor = TEL
                    Teor = re.sub(r'(\n\s*){2,}', '\n\n', Teor)
                    match_corpo = re.search(r'(Nr\.\s\d+\s)(.*)', Teor, re.DOTALL)
                    ...
                    Corpo = re.sub(r'\n\s*\n', '\n\n', Corpo)
                    Teor = re.sub(r'[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]', '', Teor)
                    Corpo = re.sub(r'[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]', '', Corpo)
                    Corpo = re.sub(r'(?<=[\w.,;!?])\n(?=[^\n])', ' ', Corpo)
